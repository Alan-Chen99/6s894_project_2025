...
That first comparison is an internal fusion ablation. For a
functionality-matched external baseline, we replace the separate transform with
the unmodified Tri Dao H16 kernel at pinned commit \texttt{e7706faf}, apply the
fixed Rademacher signs with a separate stock PyTorch BF16 multiply, and retain
unmodified TE 2.18 block FP8. The external and project RHT outputs match
bit-for-bit in BF16. Across three independently started processes, our fused
path is 2.308$\times$ faster in forward, 1.769$\times$ in backward, and
1.974$\times$ for forward plus backward; the respective ranges are
2.306--2.309$\times$, 1.767--1.775$\times$, and 1.969--1.981$\times$.
Profiling attributes 5.119 ms to four upstream H16 launches and 0.370 ms to
four separate sign multiplies, while TE GEMM time is unchanged. Relative to TE
block FP8 with RHT omitted, our path is instead 0.941$\times$/0.946$\times$ in
forward/forward-plus-backward; hence the 1.974$\times$ claim applies only to
the RHT-enabled external composition.
