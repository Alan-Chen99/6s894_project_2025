...
with Transformer Engine block-FP8 quantization. On a Llama-shaped MLP, the
...
signs, and unmodified Transformer Engine, while remaining 5.4\% slower than
Transformer Engine with the RHT omitted. Native NVFP4 results remain pending.
\end{abstract}
...
That first comparison is an internal fusion ablation. Our
...
\begin{table}[t]
\caption{Llama-shaped MLP latency on H100 NVL. The external path composes
unmodified Tri Dao H16, stock PyTorch signs, and unmodified TE block FP8.
Values are medians across three independently started processes.}
...
but remains 5.4\% slower than TE with RHT omitted. In a separate exact
