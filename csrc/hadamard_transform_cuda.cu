    ...
    hadamard_transform_kernel(b16* a, b16* out, int total_num_chunks)
{
    ...
    b32 b_frag_all[num_chunks][4]; // for all chunks, holds matrix fragment (which takes 4
                                   // regs of b16x2 * 32 threads)
    ...
    b32* a_start_ptr =
        (b32*)(a + blockid * num_chunks * 256); // offset a to where this warp starts
...
#pragma unroll
        ...
        if constexpr (log_had_size <= 4 || log_had_size % 4 == 0)
            break;
    }
...
#pragma unroll
    for (int l = 0; l < 2; l++) {
        if constexpr (log_had_size <=
                      8) { // l == 0 guaranteed, redundant simplified version of else
                           // body, to help compiler warnings
            b_frag_ptr = bfrag_arr + (blockid % warps_per_block) * num_chunks * 128;
        } else {
            b_frag_ptr = bfrag_arr +
                (blockid % warps_per_block) * num_chunks *
                    (l == 0 ? 128 : (128 >> part8_log_had_size));
        }

        if (l == 1) {
            if constexpr (log_had_size > 8) {
                __syncthreads(); // sync between first and second iterations if above size
                                 // 256
...
#pragma unroll
                        for (int k = 0; k < num_chunks; k++) {
                            // here, j represents register, and k represents
                            // 8-offset/chunk
                            int real_chunk_num =
                                (num_chunks - (threadid % num_chunks) + k) %
                                num_chunks; // chunk at which you have target thread #'s
                                            // data

                            int real_thread_id = (threadid / num_chunks) * num_chunks +
                                k; // target thread #
                            int chunk_idx = 128 *
                                real_chunk_num; // index due to fetching from another
                                                // chunk (chunk in which this thread has
                                                // the target thread's original data)
                            int thread_group_idx = (real_thread_id / 4) *
                                16; // index due to fetching from another group of
                                    // num_chunk threads (since shuffle is between
                                    // num_chunk threads)
                            int thread_idx = (real_thread_id % 4) *
                                2; // index due to original thread's position within the
                                   // group of num_chunk threads
                            int reg_idx =
...
#pragma unroll
                for (int j = 0; j < 4; j++) {
                    ...
                    b_frag_all[k][j] = b_frag_ptr
                        [(real_row + (reg % 2) * 4) + (real_col + (j / 2) * 8) * 8];
                }
...
#pragma unroll
            ...
            } else if constexpr (log_had_size >
                                 8) { // condition is redundant to help compiler warnings
                if constexpr (log_had_size < 12) {
                    // sizes 512, 1k, and 2k
...
#pragma unroll
                ...
                // for size 4k and above, wait to process all chunks so a final store can
                // be performed coalesced
            }

            a_chunk_ptr += 128; // (only affects first 256 size) move on to next chunk by
                                // skipping 256 elements in b16 (= 128 in b32)
            out_chunk_ptr += 128;
            if constexpr (log_had_size > 8) {
                b_frag_ptr += (l == 0 ? 128 : (128 >> part8_log_had_size));
            } else { // else is redundant, simplified version of if body, to help compiler
                     // warnings
                b_frag_ptr += 128;
            }
        }
...
#pragma unroll
            for (int k = 1; k < num_chunks; k++) {
                int threadid_contig = threadid % num_chunks;
                int threadid_mul = threadid / num_chunks;
                int threadid2 = (threadid_contig + k) % num_chunks +
                    threadid_mul * num_chunks; // thread to give your data to
        ...
        // can then index into all chunks owned by this warp
        b32* store = bfrag_arr +
            (128 >> part8_log_had_size) * (num_chunks * (blockid % warps_per_block));
...
#pragma unroll
            for (int k = 0; k < num_chunks; k++) {
                ...
                int real_chunk_num = (num_chunks - (threadid % num_chunks) + k) %
...
#pragma unroll
                ...
                int thread_group_idx = (real_thread_id / 4) *
                    16; // index due to fetching from another group of num_chunk threads
                        // (since shuffle is between num_chunk threads)
                int thread_idx =
                    (real_thread_id % 4) * 2; // index due to original thread's position
...
#define CHECK_SHARED_LIM() \
    ...
    if constexpr (check_masking) {
        if (num_chunks % (chunks_per_warp * warps_per_block) != 0) {
            ...
            auto kernel = hadamard_transform_kernel<
                chunks_per_warp,
                warps_per_block,
                log_had_size,
                blocks_per_sm,
...
)
{
    ...
    if (numel <= 256) {
        ...
        case (1 << 1):
            run_kernel<
                dtype,
                chunks_per_warp_small,
                warps_per_block_small,
                1,
                blocks_per_sm_small
            >(a_mat, out, num_chunks, stream);
            break;
        case (1 << 2):
            run_kernel<
                dtype,
        ...
        case (1 << 1):
            run_kernel<
                dtype,
                chunks_per_warp_large,
                warps_per_block_large,
                1,
                blocks_per_sm_large,
                true
            >(a_mat, out, num_chunks, stream);
        ...
        case (1 << 9):
            run_kernel<
                dtype,
                launch_configs_big[0][0],
                launch_configs_big[0][1],
                9,
                launch_configs_big[0][2]
            >(a_mat, out, num_chunks, stream);
            break;
        case (1 << 10):
            run_kernel<
                dtype,
                launch_configs_big[1][0],
                launch_configs_big[1][1],
                10,
                launch_configs_big[1][2]
            >(a_mat, out, num_chunks, stream);
            break;
        case (1 << 11):
            run_kernel<
                dtype,
                launch_configs_big[2][0],
                launch_configs_big[2][1],
                11,
                launch_configs_big[2][2]
            >(a_mat, out, num_chunks, stream);
            break;
        case (1 << 12):
            run_kernel<
                dtype,
                launch_configs_big[3][0],
                launch_configs_big[3][1],
                12,
                launch_configs_big[3][2]
            >(a_mat, out, num_chunks, stream);
            break;
        case (1 << 13):
            run_kernel<
                dtype,
                launch_configs_big[4][0],
                launch_configs_big[4][1],
                13,
                launch_configs_big[4][2]
            >(a_mat, out, num_chunks, stream);
            break;
        case (1 << 14):
            run_kernel<
                dtype,
                launch_configs_big[5][0],
                launch_configs_big[5][1],
                14,
                launch_configs_big[5][2]
            >(a_mat, out, num_chunks, stream);
            break;
        case (1 << 15):
            run_kernel<
                dtype,
                launch_configs_big[6][0],
                launch_configs_big[6][1],
                15,
                launch_configs_big[6][2]
            >(a_mat, out, num_chunks, stream);
            break;
        }
    }
